...
private:
 ...
 //begin added  by xiaodong.liu for task7046375 visidon alg 2018/10/11
    static const char KEY_ALGORITHMS_MODE[];
    bool is3rdAlgorithmsEnable;
    int32_t set3rdAlgorithmsMode(QCameraParameters& params);
 #ifdef TCT_VISIDON_SOLUTION
        static const char KEY_VISIDON_MODE[];
        static const char MODE_VISIDON_OFF[];
        static const char MODE_VISIDON_FACE_BEAUTY[];
        #ifdef VISIDON_LOWLIGHT
        static const char MODE_VISIDON_LOW_LIGHT[];
        #endif // VISIDON_LOWLIGHT

        #ifdef VISIDON_SUPERPHOTO
        static const char MODE_VISIDON_SUPER_PHOTO[];
        uint32_t getZoomRatio() const;
        #endif // VISIDON_SUPERPHOTO

       static const char KEY_VISIDON_SKIN_SMOOTHING[]; // 0~100
       static const char KEY_VISIDON_SKIN_TONEADJUSTING[]; // 0~100
       int32_t setVisidonMode(QCameraParameters& params);
       void setVisidonMode(const char *str);
       int32_t setVisidonSkinSmoothing(QCameraParameters& params);
       void setVisidonSkinSmoothing(const char *str);
       int32_t setVisidonSkinToneAdjusting(QCameraParameters& params);
       void setVisidonSkinToneAdjusting(const char *str);
       ...
       visidon_state_type visidon_state;
      #endif // VISIDON_LOWLIGHT||VISIDON_SUPERPHOTO

      visidon_mode_type visidon_mode;
      uint8_t visidon_skin_smooting;
      uint8_t visidon_skin_toneAdjusting;
...
public:
  ...
  int32_t getStreamSubFormat(cam_stream_type_t streamType,
    ...
    visidon_mode_type getVisidonMode();
    void getVisidonParameters(visidon_parameters_t *VisidonParameters);
    bool getAlgorithmsMode();
